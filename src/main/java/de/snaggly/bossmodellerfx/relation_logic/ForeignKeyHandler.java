...
public class ForeignKeyHandler {
    ...
    public static void addForeignKeys(Relation relation) {
        ...
        && relation.getTableB_Cardinality() == CrowsFootOptions.Cardinality.ONE) {
            ...
            && relation.getTableB_Obligation() == CrowsFootOptions.Obligation.MUST) {
                //Can-Must -> FK to TabA
                performAddForeignKeys(relation.getTableA(), relation.getTableB(), true, true);
            }
                ...
                //Must-Can -> FK to TabB
                performAddForeignKeys(relation.getTableB(), relation.getTableA(), true, true);
            }
            else if (relation.getTableA_Obligation() == CrowsFootOptions.Obligation.MUST
                    && relation.getTableB_Obligation() == CrowsFootOptions.Obligation.MUST) {
            ...
            && relation.getTableB_Obligation() == CrowsFootOptions.Obligation.CAN) {
                ...
                performAddForeignKeys(relation.getTableB(), relation.getTableA(), false, false);
            }
            else if (relation.getTableA_Obligation() == CrowsFootOptions.Obligation.MUST
                    && relation.getTableB_Obligation() == CrowsFootOptions.Obligation.CAN) {
                //Must-Can
                performAddForeignKeys(relation.getTableB(), relation.getTableA(), true, false);
            }
        }
            ...
            && relation.getTableB_Obligation() == CrowsFootOptions.Obligation.CAN) {
                GUIMethods.showInfo("Relations Editor", "Transformation", "Gewählte Relation könnte Transformiert werden!");
                performAddForeignKeys(relation.getTableA(), relation.getTableB(), false, false);
            }
            else if (relation.getTableA_Obligation() == CrowsFootOptions.Obligation.CAN
                    && relation.getTableB_Obligation() == CrowsFootOptions.Obligation.MUST) {
                //Can-Must
                performAddForeignKeys(relation.getTableA(), relation.getTableB(), true, false);
            }
        }
    }
    ...
    private static void performAddForeignKeys(Entity tableToAdd, Entity tableFKeyReferencesTo, boolean isMust, boolean isUnique) {
